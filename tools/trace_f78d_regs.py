#!/usr/bin/env python3
"""Trace what happens when PC hits F78D area."""
import json
import sys
...
def main():
    ...
    for index in range(max_steps):
        ...
        if (cs == 0xF000) and (ip >= 0xF780) and (ip <= 0xF7A0):
            ...
            if count % 25 == 0:
                ax = fmt(trace['after'].get('AX', '?'))
                bx = fmt(trace['after'].get('BX', '?'))
                cx = fmt(trace['after'].get('CX', '?'))
                dx = fmt(trace['after'].get('DX', '?'))
                sp = fmt(trace['after'].get('SP', '?'))
                bp = fmt(trace['after'].get('BP', '?'))
                si = fmt(trace['after'].get('SI', '?'))
                di = fmt(trace['after'].get('DI', '?'))
                flags_hex = format(trace['after'].get('FLAGS', 0), '04X')
                print(f"[{index}] @{cs:04X}:{ip:04X} x{count} AX={ax} BX={bx} CX={cx} DX={dx} SP={sp} BP={bp} SI={si} DI={di} FL={flags_hex}")
            ...
            if state_key[1] == 0xF78D and count % 5 == 0:
                ...
                print(f"  CS:{ip:04X} AX={ax} BX={bx} CX={cx} DX={dx} "
                      f"SP={sp} BP={bp} SI={si} DI={di} FLAGS={format(trace['after'].get('FLAGS', 0), '04X')}")
