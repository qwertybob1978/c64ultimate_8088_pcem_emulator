...
def safe_byte(data, pos):
    """Read one byte safely; return None if out of bounds."""
    try:
        return data[pos]
    except IndexError:
        return None

def safe_word(data, pos):
    """Read two bytes as little-endian word; return None if out of bounds."""
    b0 = safe_byte(data, pos)
    b1 = safe_byte(data, pos + 1)
...
def disassemble_rom(rom_bytes, start_offset, max_instructions=200):
    """Disassemble x86 real-mode bytes starting from offset in ROM."""
    ...
    while ip < len(rom_bytes) and len(results) < max_instructions:
        ...
        # Safety: ensure opcode exists
        opcode_val = safe_byte(rom_bytes, ip)
        ...
        # OUT imm8,AL (E6 xx)
            ...
            port = safe_byte(rom_bytes, ip); ip += 1
        ...
        # RET (C3) / RET imm16 (C2 xx xx)
            ...
            ret_count = safe_word(rom_bytes, ip); ip += 2
        ...
        # MOV r/m,reg (8A) or MOV reg,r/m (8B)
            ...
            modrm = safe_byte(rom_bytes, ip); ip += 1
            ...
            if mod == 3:
                ...
                if mod == 1:
                    disp8 = safe_byte(rom_bytes, ip); ip += 1
                    base_str = addr_map.get(str(rm_field), "?")
                    mem_op = f"[{base_str}+{disp8:#X}]"
                elif mod == 2:
                    disp16_val = safe_word(rom_bytes, ip); ip += 2
                    base_str = addr_map.get(str(rm_field), "?")
        ...
        # MOV AL/EAX,moffs (A0-A1) or mov moffs,AL/AX (A2-A3)
            ...
            offset_val = safe_word(rom_bytes, ip); ip += 2
        ...
        # ADD AL,immediate (04 xx) / OR AX,immediate (0D xx xx)
            ...
            imm = safe_byte(rom_bytes, ip); ip += 1
            if imm is None: break
            mnemonic = "ADD_AL_im"; operands = f"{imm:#X}"
        
        elif opcode == 0x0D:
            val = safe_word(rom_bytes, ip); ip += 2
        ...
        # INT imm8 (CD xx)
            ...
            vec = safe_byte(rom_bytes, ip); ip += 1
...
def main():
    ...
    lines = disassemble_rom(rom, start_offset, max_instructions=150)
