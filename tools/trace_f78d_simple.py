#!/usr/bin/env python3
"""Simple trace around F78D loop region."""
import json
    ...
    for index in range(max_steps):
        ...
        if (cs == 0xF000) and (ip >= 0xF780) and (ip <= 0xF7A0):
            ...
            if count % 25 == 0:
                ...
                print(f"[{index}] @{cs:04X}:{ip:04X} x{count} AX={ax} BX={bx} CX={cx} DX={dx} SP={sp} FL={flags_hex}")
            ...
            if state_key[1] == 0xF78D and count % 5 == 0:
                ...
                print(f"  CS:{ip:04X} AX={fmt(trace['after'].get('AX','?'))} "
                      f"BH={trace['after'].get('BH','?')} BL={trace['after'].get('BL','?')} "
                      f"CX={fmt(trace['after'].get('CX','?'))} FLAGS={format(trace['after'].get('FLAGS', 0), '04X')}")
