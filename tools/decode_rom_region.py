#!/usr/bin/env python3
"""Decode BIOS ROM bytes at physical addresses around FF78D."""
import sys
from pathlib import Path

# Read the guest image
with open(r'C:\Repository\C64_x86\build\guest-genxt.reu', 'rb') as f:
    data = f.read()

# Physical address 0xFF78D maps to file offset 0xFF78D - 0xFE000 = 0x178D
rom_offset_start = 0x1780
rom_offset_end = rom_offset_start + 256

print(f"Decoding BIOS ROM from physical address 0xF7800+0x1780=0xF78D")
print(f"(File offset {rom_offset_start:#06X} to {rom_offset_end:#06X})\n")

pos = rom_offset_start
while pos < min(rom_offset_end, len(data)):
...
print("\n\n=== Full x86 opcode decode of ROM region ===")
pos = rom_offset_start
while pos < min(rom_offset_end, len(data)):
    addr = 0xFE000 + pos
    opcode = data[pos]
    ...
    # Simple x86 opcode decoder for common opcodes
    mnemonics = {
